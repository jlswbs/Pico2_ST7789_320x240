// Voronoi 4 distribution (cell noise) //
...
void loop() {
  ...
  for (int j=0; j<HEIGHT; j++) {
    ...
    for (int i=0; i<WIDTH; i++) {
      ...
      for (int p=0; p<PARTICLES; p++) if (distance(x[p], y[p], i, j) < mindist) mindist = distance(x[p], y[p], i, j);

      uint8_t coll1 = 255 * sinf(mindist); 
      uint8_t coll2 = 255 * sinf(mindist);
      uint8_t coll3 = 255 * logf(mindist);

      col[((2*i)+(2*j)*FULLW)] = color565(coll1, coll2, coll3);
